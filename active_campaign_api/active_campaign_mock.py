...
def mock_active_campaign(requests_mock) -> typing.Callable:  # noqa
    ...
    def _mock_active_campaign() -> None:  # noqa
        ...
        def req_json_value(req: requests.PreparedRequest, value: str) -> str:
        ...
        def write_contact_callback(
            ...
            return {"contact": {"email": recieved_email, "id": contact_id}}

        requests_mock.post(
            f"{root_url}/contacts", json=write_contact_callback, status_code=201
        )
        ...
        def find_tag_callback(
            request: requests.PreparedRequest,
            context: typing.Callable,
        ) -> dict:
            parsed_url = urlparse(request.url)
            query_dict = (
                parse_qs(parsed_url.query) if isinstance(parsed_url.query, str) else {}
            )
            search_query = (
                query_dict["search"][0] if "search" in query_dict else "example tag"
            )
            return {
                "tags": [
                    {"tagType": "contact", "tag": search_query, "description": ""}
                ],
                "meta": {"total": 1},
            }

        # without query params so as to match everything
        requests_mock.get(f"{root_url}/tags", json=find_tag_callback)

        def find_list_callback(
            request: requests.PreparedRequest,
            context: typing.Callable,
        ) -> dict:
            parsed_url = urlparse(request.url)
            ...
            name_query = (
                query_dict["filters[name]"][0]
                if "filters[name]" in query_dict
                else "example list"
            )
            list_id = (
                ActiveCampaignAPI.LISTS[name_query]
        ...
        # without query params so as to match everything
        requests_mock.get(f"{root_url}/lists", json=find_list_callback)
        ...
        requests_mock.post(f"{root_url}/contactLists", json=update_contact_list)

        def create_contact_tag_callback(
            request: requests.PreparedRequest,
            context: typing.Callable,
        ) -> dict:
            ...
            return {"contactTag": {"id": 1, "contact": contact_id, "tag": tag_id}}

        requests_mock.post(
            f"{root_url}/contactTags", json=create_contact_tag_callback, status_code=201
        )
